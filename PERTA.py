...
class PertaLayer(nn.Module):
    def __init__(self, bert_layer,
                 input_dim,
                 prompt_len,
                 bottleneck_dim,opt_dim=None):
        super(PertaLayer, self).__init__()
    ...
    def forward(
        ...
        return (outputs[0][:,self.prompt_len:,:],)
...
class PertaEncoder(nn.Module):
    def __init__(self, bert_encoder,
                 input_dim,
                 prompt_len,
                 bottleneck_dim,opt_dim=None):
        super(PertaEncoder, self).__init__()
        ...
        self.layer = nn.ModuleList([PertaLayer(bert_encoder.layer[i],
                                               self.input_dim,
                                               self.prompt_len,
                                               self.bottleneck_dim,
...
class PERTA(nn.Module):
    def __init__(self, pretrained_bert,
        ...
        self.encoder = PertaEncoder(pretrained_bert.encoder,
                                    self.hidden_size,
                                    self.prompt_len,
                                    self.bottleneck_dim,
                                    self.opt_dim)
...
if __name__=='__main__':
    ...
    perta=PERTA(pretrained_bert,2,8,64)
